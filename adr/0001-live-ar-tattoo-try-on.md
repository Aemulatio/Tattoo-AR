...
| Phase 3 — forearm surface        | In progress | On `phase-3-forearm-surface`: side selection, stable local frame, continuous roll/confidence, reusable 12×24 tapered mesh, UVs, wireframe, seam, callback-local mask radius sampling, temporal radius stabilization, anatomical fallback, and measurable flip/radius acceptance telemetry implemented; browser check passed, Android/iOS evidence remains |
