...
| Phase 5 — upload and placement   | In progress | Local PNG/JPEG picker, file-size and byte-signature validation, orientation-aware decoding with browser fallback, 2048 px downscaling, disposable canvas textures, camera-restart-safe asset ownership, accessible status/error messaging, upload retry, white-paper threshold/feather/invert processing, and independent ink opacity implemented; desktop and iPhone manual checks passed |
